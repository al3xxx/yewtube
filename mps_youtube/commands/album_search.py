...
from .. import c, g, screen, __version__, __url__, config, util, extractor
...
def _best_song_match(songs, title, duration, titleweight, durationweight):
    ...
    for song in songs:
        ...
        util.dbg(
            "Title score: %s, Duration score: %s", title_score, duration_score
        )
...
def _match_tracks(artist, title, mb_tracks):
    ...
    for track in mb_tracks:
        ...
        # perform fetch
        wdata = extractor.search_videos(
            q, int(config.PAGES.get)
        )  # extractor.call_gdata('search', query)
        ...
        s, score = _best_song_match(
            results, artist + " " + ttitle, length, 0.5, 0.5
        )
