...
@command(
    r"(dv|da|d|dl|download)\s*(\d{1,4})", "da", "dv", "d", "dl", "download"
)
def download(dltype, num):
    ...
    try:
        ...
        if success:
            g.message = (
                "Saved '" + song.title + "' to " + c.g + config.DDIR.get + c.w
            )
...
def _make_fname(song, ext=None, av=None, subdir=None):
    ...
    if not ext:
        stream = streams.select(
            streams.get(song), audio=av == "audio", m4a_ok=True
        )
        ext = stream["ext"]
...
def get_dl_data(song, mediatype="any"):
    ...
    for n, stream in enumerate(streamlist):
        sys.stdout.write(text + "-" * n + ">" + " " * (length - n - 1) + "<\r")
        sys.stdout.flush()
