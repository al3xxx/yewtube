...
class Mpris2MediaPlayer(dbus.service.Object):
        ...
        if name == "pause":
            ...
            if newval != oldval:
                ...
                self.PropertiesChanged(
                    PLAYER_INTERFACE, {"PlaybackStatus": newval}, []
                )
            ...
            if newval != oldval:
                self.properties[PLAYER_INTERFACE]["read_write"]["Volume"] = (
                    newval
                )
            ...
            newval = {
                "mpris:trackid": dbus.ObjectPath(
                    "/CurrentPlaylist/ytid/" + trackid_sanitized,
                    variant_level=1,
                ),
            ...
            if newval != oldval:
                ...
                self.PropertiesChanged(
                    PLAYER_INTERFACE, {"Metadata": newval}, []
                )
    ...
    @dbus.service.method(
        dbus_interface=PROPERTIES_INTERFACE, in_signature="ssv"
    )
        ...
        if interface_name in self.properties:
            if property_name in self.properties[interface_name]["read_write"]:
                if property_name == "Volume":
                    self._sendcommand(
                        ["set_property", "volume", new_value * 100]
                    )
    ...
    @dbus.service.signal(
        dbus_interface=PROPERTIES_INTERFACE, signature="sa{sv}as"
    )
