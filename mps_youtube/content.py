...
def generate_songlist_display(song=False, zeromsg=None):
    ...
    for n, x in enumerate(g.model[:max_results]):
        ...
        # details['category'] = 'extractor.get_categoryname(cat)'
        details["ytid"] = x.ytid
        line = ""
...
def _get_user_columns():
    ...
    defaults = {
        ...
        "date": dict(name="uploaded", size=14, heading="Date"),
        "time": dict(name="uploaded", size=14, heading="Time"),
...
def get_last_query():
    ...
    if (
        ...
        or "func" not in g.last_search_query[1]
    ):
        g.content = logo()
