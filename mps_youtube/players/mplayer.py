...
class mplayer(CmdPlayer):
    ...
    def launch_player(self, cmd):
        ...
        if g.mprisctl:
            self.temp_dir = tempfile.mkdtemp(prefix="mpsyt-mplayer-")
            self.fifopath = os.path.join(self.temp_dir, "mplayer.fifo")
...
def _get_mplayer_version(exename: str) -> T.Union[int, T.Tuple[int, ...]]:
    ...
    m = re.search(
        r"MPlayer \S*?SVN[\s-]r([0-9]+)", o, re.MULTILINE | re.IGNORECASE
    )
