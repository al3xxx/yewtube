...
class mpv(CmdPlayer):
    ...
    def _generate_real_playerargs(self):
        ...
        completetitle = "=".join((pd["title"], self.song.title))
    ...
    def launch_player(self, cmd):
        ...
        if self.mpv_usesock:
            ...
            t = threading.Thread(target=capture_stderr, args=(self.p.stderr,))
            t.daemon = True
            t.start()
        ...
        util.dbg("mpv exited with code %s", returncode)

        if returncode != 0 and returncode not in (42, 43):
            # Try to find meaningful error messages
            interesting = [
                l
                for l in self.error_log
                if "Error" in l or "failed" in l or "[ffmpeg]" in l
            ]
            util.dbg(
                "Captured %s log lines, %s interesting",
                len(self.error_log),
                len(interesting),
            )
            if not interesting:
                interesting = self.error_log[-5:]

            if interesting:
                g.message = (
                    f"{c.r}mpv error ({returncode}): {interesting[0]}{c.w}"
                )
        ...
        if self.sockpath:
            ...
            while tries < 10 and self.p.poll() is None:
                ...
                if self.p.poll() is not None:
                    g.message = (
                        c.r
                        + "mpv failed to start. Check stderr for details."
                        + c.w
                    )
                return
